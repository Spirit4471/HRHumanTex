...
class RGB2Texture:
    ...
    def generate_uv_texture(self):
        ...
        if not os.path.exists(output_textures_folder_path):
        ...
        if not os.path.exists(output_debug_folder_path):
        ...
        files = [f for f in files if not f.startswith('.') and f.lower().endswith(('.jpg', '.png', '.jpeg'))]
        ...
        for path in files:
            ...
            current_iuv_path = os.path.join(dataset_iuv_path, f"{filename_wo_ext}_densepose.png")
            if os.path.isfile(current_image_path) and os.path.isfile(current_iuv_path):
                images_file_paths.append(current_image_path)
                images_iuv_paths.append(current_iuv_path)
            else:
                print(current_image_path, "or", current_iuv_path, "does not exist")

        images_file_paths.sort()
        images_iuv_paths.sort()

        for idx, (current_image_path, current_iuv_path) in enumerate(zip(images_file_paths, images_iuv_paths)):
            print('\nComputing UV texture ', idx+1, '/', len(images_file_paths))
            ...
            normal_tex, _ = converter.convert((tex_trans*255).astype('int'), mask=mask_trans)

            # 
            import cv2
            
            normal_np = (normal_tex * 255).round().astype(np.uint8)
            kernel = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (3,3))
            # 
            channels = cv2.split(normal_np)
            closed_channels = [cv2.morphologyEx(ch, cv2.MORPH_CLOSE, kernel) for ch in channels]
            closed_tex = cv2.merge(closed_channels)
            # 
            closed_tex = closed_tex.astype(np.float32) / 255.0

            # 
            fig, (ax1, ax2, ax3) = plt.subplots(1,3,figsize=(13,4))
            ax1.imshow(closed_tex)
            ...
            # 
            debug_filename = os.path.basename(current_image_path).replace(".jpg", "_debug.png")
            debug_filepath = os.path.join(output_debug_folder_path, debug_filename)
            plt.savefig(debug_filepath, dpi=150)
            plt.close(fig) 

            # 
            output_filename = os.path.basename(current_image_path).replace(".jpg", "_texture.png")
            output_filepath = os.path.join(output_textures_folder_path, output_filename)
            # 
            im_out = Image.fromarray((closed_tex * 255).round().astype(np.uint8), 'RGB')
            im_out.save(output_filepath)
            print('Saved texture to', output_filepath)
    ...
    def compute_mask_of_partial_uv_textures(self):
            ...
            if not os.path.exists(output_masks_folder_path):
            ...
            for image_filename in files:
                if not image_filename.lower().endswith(tuple(valid_exts)):
                    continue 
                image_path = os.path.join(uv_textures_path, image_filename)
                if not os.path.isfile(image_path):
                    continue
                print('\nComputing UV mask for', image_filename)
                current_image = skimage.io.imread(image_path)
                # 
                mask = (current_image[:, :, 0] < 2) & (current_image[:, :, 1] < 2) & (current_image[:, :, 2] < 2)
                ...
                output_path = os.path.join(output_masks_folder_path, mask_filename)
                cv2.imwrite(output_path, mask_clean)
